...
class _MainRoutineScreenState extends State<MainRoutineScreen> {
  ...
  List<TaskModel> _getUpcomingTasks(RoutineStateModel model) {
    final currentIndex = model.currentTaskIndex;
    if (currentIndex >= model.tasks.length - 1) {
      // No upcoming tasks
      return [];
    }
    // Return tasks after current task
    return model.tasks.sublist(currentIndex + 1);
  }
        ...
        return Scaffold(
          ...
          body: Stack(
            children: [
              ...
              // Task drawer for upcoming tasks
              TaskDrawer(
                upcomingTasks: _getUpcomingTasks(model),
